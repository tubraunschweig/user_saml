 ...
 */
class Version4200Date20211025124248 extends SimpleMigrationStep {
	...
	public function postSchemaChange(IOutput $output, Closure $schemaClosure, array $options): void {
		...
		foreach ($prefixes as $prefix) {
			...
			while ($row = $this->readConfiguration($configKeys)) {
				$configData[$this->normalizeKey($row['configKey'])] = $row['configValue'];
				$keysToDelete = $row['configKey'];
			}

			if ($this->insertConfiguration($prefix, $configData)) {
