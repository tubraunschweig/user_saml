...
class SAMLSettings {
	...
	public function getOneLoginSettingsArray(int $idp): array {
		...
		$settings = [
			...
			'idp' => [
				...
				'singleSignOnService' => [
					'url' => $this->configurations[$idp]['idp-singleSignOnService'] ?? '',
				],
		...
		if (($this->configurations[$idp]['idp-singleLogoutService.url'] ?? '') !== '') {
			$settings['idp']['singleLogoutService'] = $this->configurations[$idp]['idp-singleLogoutService.url'];
			$settings['sp']['singleLogoutService'] = $this->urlGenerator->linkToRouteAbsolute('user_saml.SAML.singleLogoutService');
	...
	public function getProviderId(): int {
		return (int)$this->session->get('user_saml.Idp');
	}
